...
public class SpreadView extends View {
    ...
    private int distance = 5; //每次圆递增间距//5
    private int maxRadius = 10; //最大圆半径  80
    private int delayMilliseconds = 200;//扩散延迟间隔，越大扩散越慢 33
    ...
    public SpreadView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        int centerColor = a.getColor(R.styleable.SpreadView_spread_center_color, ContextCompat.getColor(context, R.color.colorAccent1));
        int spreadColor = a.getColor(R.styleable.SpreadView_spread_spread_color, ContextCompat.getColor(context, R.color.colorSpread));
        ...
        centerPaint.setStrokeWidth(3); //宽度
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < spreadRadius.size(); i++) {
            ...
            if (alpha > 0 && width <300) {
                alpha = alpha - distance > 0 ? alpha - distance : 5;
                alphas.set(i, alpha);
